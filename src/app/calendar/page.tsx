  ...
  useEffect(() => {
    async function fetchCalendars() {
      try {
        ...
        // Filter out only the calendar items from all notifications
    ...
    <div className="container" style={{ padding: '4rem 0' }}>
      ...
      <div className="card" style={{ padding: '2rem' }}>
        <p className="card-text" style={{ fontSize: '1.1rem', marginBottom: '2rem' }}>
          Welcome to the Directorate of Academic Audit and Planning (DAAP) calendar portal. Here you can find the official, live academic schedules, examination dates, and holidays for all constituent and affiliated colleges under JNTU-GV.
        </p>
        ...
        {loading ? (
          <div style={{ textAlign: 'center', padding: '4rem 2rem', color: '#64748b' }}>
            ...
            <p>Fetching official academic calendars from JNTU-GV...</p>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', backgroundColor: 'white' }}>
              <thead>
                <tr style={{ backgroundColor: '#2D1B5E', color: 'white' }}>
                  <th style={{ padding: '1rem', textAlign: 'center', fontWeight: '600', width: '15%' }}>Date</th>
                  <th style={{ padding: '1rem', textAlign: 'left', fontWeight: '600', width: '65%' }}>Calendar Title</th>
              ...
              <tbody>
                {filteredCalendars.length > 0 ? (
                  filteredCalendars.map((cal, index) => (
                    <tr key={cal.id} style={{ borderBottom: '1px solid #e2e8f0', backgroundColor: index % 2 === 0 ? 'white' : '#f8fafc', transition: 'background-color 0.2s' }}>
                      <td style={{ padding: '1rem', textAlign: 'center' }}>
                        <div style={{ display: 'inline-flex', flexDirection: 'column', alignItems: 'center', backgroundColor: '#f1f5f9', borderRadius: '0.375rem', padding: '0.25rem 0.75rem', border: '1px solid #e2e8f0' }}>
                          <span style={{ fontSize: '1.25rem', fontWeight: '700', color: 'var(--primary)' }}>{cal.day}</span>
                          <span style={{ fontSize: '0.8rem', fontWeight: '600', color: '#64748b', textTransform: 'uppercase' }}>{cal.month} {cal.year}</span>
                        </div>
                      </td>
                  ...
                  <tr>
                    <td colSpan={3} style={{ padding: '3rem', textAlign: 'center', color: '#64748b' }}>No calendars found matching your search.</td>
                  </tr>
                )}
              </tbody>
            </table>
